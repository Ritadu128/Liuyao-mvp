...
| 手势投掷 | MediaPipe 识别握拳/张掌，含稳定时间、置信度、冷却和权限错误提示；模型及运行文件由本站同域提供。 | 已修复摄像头播放与模型启动时序；HTTPS 真机待复验。 |
...
│   │   ├── assets/                        # 铜钱贴图
│   │   ├── data/                          # 64 卦映射和经文 JSON
│   │   ├── mediapipe/                     # 同域手势模型与 WASM 运行文件
│   │   └── support/                       # 作者明确授权公开的微信、支付宝收款码
│   └── src/
│       ├── App.tsx                        # 页面路由
│       ├── components/
│       │   ├── CoinScene.tsx              # Three.js 硬币动画
│       │   ├── GestureThrowControls.tsx   # 极简手势入口、状态与蓄力条
...
| `DEEPSEEK_TIMEOUT_MS` | 否 | 服务器钳制到 3000–60000ms，默认 60000ms；页面会在此期间持续接收流式文字。 |
...
用户必须主动点击“手势投掷”并在浏览器中授权摄像头。权限被拒、没有摄像头、设备被占用、浏览器不支持、画面未真正开始播放或模型启动超时时，界面会显示对应中文原因。摄像头仅由浏览器端 MediaPipe GestureRecognizer 使用；视频不展示、不上传，停止识别或离开投掷页时会关闭媒体轨道。手势模型和 WASM 随网站从同域加载，避免用户启动时额外依赖第三方资源地址。首次启动需下载约 20 MB，模型与对应 SIMD/WASM 运行文件并行获取；固定版本路径使用长期浏览器缓存，后续启动无需重复下载。

| 阶段 | 判定 | 用户反馈 | 保护 |
|---|---|---|---|
| 未启动 | 未取得媒体流 | HUD 内“手势投掷”入口 | 不预加载模型。 |
| 启动中 | 已请求权限，等待真实视频帧与模型 | 显示启动阶段、已等待秒数与首次下载说明 | 大文件并行下载并按版本缓存；分别设置摄像头、GPU 和模型超时，允许取消。 |
| 就绪 | 摄像头与模型就绪 | 主按钮上方显示“请握拳蓄力” | 不展示预览；仅接受置信度 ≥ 0.65 的手势。 |
| 蓄力 | `Closed_Fist` 稳定 ≥ 160ms | 单行提示与 0–100% 蓄力条 | 手势丢失或异常会取消蓄力。 |
| 释放 | 蓄力后 `Open_Palm` 稳定 ≥ 80ms 且蓄力 ≥ 250ms | “张开手掌投掷” | 对张掌采用置信度滞回，复用原有单爻状态机并拒绝动画中的重复触发。 |

## Beta 验收与正式发布

后续功能和 UI 修改不得直接推送到 `main`。固定流程如下：

1. 在 `codex/*` 功能分支完成修改、测试和构建。
2. 将待验收版本更新到 `beta` 分支，由 Railway 的独立 `beta` 环境自动部署。
3. 在 Beta 域名完成桌面端、移动端、摄像头、流式解读及长图导出验收。
4. 只有项目作者明确确认通过后，才将同一已验收提交合并到 `main`，触发正式环境部署。

Railway `beta` 环境必须与 `production` 使用隔离的服务、私有网络和数据库，不让测试数据、限流记录或错误配置影响正式站。Beta 与正式环境的 Secret 只在 Railway 变量中维护，不写入 GitHub。
...
网页底部及结果附近都有政策入口：`/privacy` 为《隐私政策》，`/disclaimer` 为《免责声明》。政策以当前真实数据流为准，明确说明：问题/卦象/经文会在解读时传至服务器和 DeepSeek；匿名历史在 localStorage；服务器处理 IP 限额；摄像头在本地端处理；手势模型由本站同域提供；以及 Google Fonts、DeepSeek 和 Ko-fi 的第三方边界。
