...
用户必须主动点击“手势投掷”并在浏览器中授权摄像头。权限被拒、没有摄像头、设备被占用、浏览器不支持、画面未真正开始播放或模型启动超时时，界面会显示对应中文原因。摄像头仅由浏览器端 MediaPipe GestureRecognizer 使用；视频不展示、不上传，停止识别或离开投掷页时会关闭媒体轨道。手势模型和 WASM 随网站从同域加载，避免用户启动时额外依赖第三方资源地址。

| 阶段 | 判定 | 用户反馈 | 保护 |
|---|---|---|---|
| 未启动 | 未取得媒体流 | HUD 内“手势投掷”入口 | 不预加载模型。 |
| 启动中 | 已请求权限，等待真实视频帧与模型 | “正在启动摄像头”或“摄像头已开启，正在准备手势识别” | 分别设置摄像头和模型超时，并允许取消。 |
