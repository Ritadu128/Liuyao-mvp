...
| 生产真实 DeepSeek 成功响应 | 作者本人在 Railway production Service Variables 设置并应用 `DEEPSEEK_API_KEY` 后，正式同源 `reading.generate` 的一次最小有效请求返回 HTTP 200，且含两项解读字段。 | 仅检查 HTTP 状态、`integratedReading` 与 `hexagramReading` 的存在性；未记录问题全文、模型输出、授权头或 Secret。 |
...
| 编号 | 项目 | 处理计划 |
|---|---|---|
| SEC-09 | DeepSeek 真实上游异常响应 | 真实成功响应已在生产验收。未来变更模型、调用库或错误处理时，应以不记录授权头、问题正文或模型输出的方式复测超时、非 2xx 与异常结构响应；不得为测试而暴露或替换生产 Secret。 |
