...
| 自动部署实测 | 先前空提交 `9f9371f` 未触发部署；重新检查确认当时 Railway 的 `Auto deploy` 开关处于关闭状态。 | 用户确认后已开启 `Auto deploys when pushed to GitHub`；来源连接与自动部署现已处于启用状态，但仍需以一个真实内容变更的 Git 推送复验。 |
...
| 临时公网域名 | 已生成 `https://liuyao-mvp-production.up.railway.app`，目标端口为平台注入的 8080。 | 仅用于正式域名绑定前的验收；尚未触碰 Cloudflare DNS。 |
...
> 本记录不包含任何密码、Token、数据库连接串、API Key 或私有日志内容。临时域名已完成公开可达、数据库就绪、法务路由、HTTPS 重定向、无 DeepSeek Key 受控响应、跨站写入拒绝与 tRPC 输入校验抽检；GitHub 最小授权、仓库可见性和自动部署开关均已恢复，仍需以一个真实内容变更的 Git 推送复验。真机摄像头/分享、备份、成本控制和 `liuyao.win` 自定义域名仍需用户确认；DeepSeek Secret 仍需用户本人填写。
